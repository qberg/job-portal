...
// social-import). Creds absent in production refuses the Fake; dev/tests warn once.
...
export const createFakeSms = (): SendSms => {
  return (phoneNumber, message) => {
    // biome-ignore lint/suspicious/noConsole: development Fake adapter
    console.log(`[FAKE SMS] To ${phoneNumber}: ${message}`);
  };
};

export const resolveSms = (): SendSms => {
  const fake = createFakeSms;

  return resolveAdapter({
    credsPresent: Boolean(env.SMS_PROVIDER),
    fake,
    real: () => {
